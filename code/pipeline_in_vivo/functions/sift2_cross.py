...
def sift2_cross(derivatives_dir,sub,ses,ntcks,fixel_metric,reg_basis_cross=None,reg_fn_cross=None,reg_strength_cross=None,surgical=False,skip_sift2=False):
    ...
    out_path = f"ses-average/weights/sift2_cross/{ses}/{fixel_metric}/reg_basis_cross_{reg_basis_cross}/reg_fn_cross_{reg_fn_cross}/reg_strength_cross_{reg_strength_cross}"
    ...
    # Create streamline assignments to nodes
    run_command(f"tck2connectome"
        ...
        f" -out_assignments {ses}/tcks/tracks_{ntcks}_session_assignments.txt"
        f" -symmetric"
        f" -force"
        )
    ...
    if not skip_sift2:
        ...
        run_command(f"{cmd_tcksift2}"
            ...
            f" -streamline_groups {ses}/tcks/tracks_{ntcks}_session_assignments.txt"
            f" -reg_basis_abs {reg_basis_cross}"
            f" -reg_fn_abs {reg_fn_cross}"
            f" -reg_strength_abs {reg_strength_cross}"
    ...
    if surgical:
        run_command(f"tck2connectome"
            ...
            f" -tck_weights_in {out_path}/sift2_{ntcks}_weights{'_intersected_eq_0' if ses == ses1 else ''}.txt"
            f" -symmetric"
            f" -force"
            )
    else:
