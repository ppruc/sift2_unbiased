...
from sift2_init import sift2_init
...
for sub in subs:
    for ses in get_sessions(derivatives_dir, sub):
        ...
        prepare_anat(derivatives_dir, sub, ses, skip_coreg=True) # preprocess/register T1w to DWI
    robust_template(derivatives_dir, sub, final_template=True) # rigid T1w within-subject template registration
    wmfod_template(derivatives_dir, sub, 'rigid') # apply transforms to WMFODs and generate template

# Longitudinal FreeSurfer Pipeline run in parallel
Parallel(n_jobs=15, backend='multiprocessing')(delayed(recon_cross)(derivatives_dir, sub, ses,remove_old_run=False) for sub, ses in tasks)
Parallel(n_jobs=15, backend='multiprocessing')(delayed(recon_base_autorecon_1)(derivatives_dir, sub, remove_old_run=True) for sub in subs)
Parallel(n_jobs=15, backend='multiprocessing')(delayed(recon_base_autorecon_2_3)(derivatives_dir, sub) for sub in subs)
...
for sub in subs:
    for ses in get_sessions(derivatives_dir, sub):
       ...
       atlas_session(derivatives_dir, dependencies_dir, sub, ses) # derive parcellations for sessions
    fixels_template(derivatives_dir, sub,'rigid') # segment fixels for template
    tck_template(derivatives_dir, sub, ntcks) # run template tractography
    atlas_template(derivatives_dir, dependencies_dir, sub) # derive parcellation for template
    ...
    for ses in get_sessions(derivatives_dir, sub):
        ...
        # Unbiased Tractogram  
        sift2_template(derivatives_dir, sub, ntcks, "fd",
        ...
        # Pipeline 3
        for ses in get_sessions(derivatives_dir, sub):
            sift2_init(derivatives_dir, sub, ses, ntcks, "fd",
            reg_fn_temp=reg_fn_temp,
            reg_basis_temp=reg_basis_temp,
            reg_strength_temp=reg_strength_temp,
        ...
        # Pipeline 4
        sift2_differential(derivatives_dir, sub, ntcks, "fd",
            reg_fn_temp=reg_fn_temp,
            reg_basis_temp=reg_basis_temp,
            reg_strength_temp=reg_strength_temp,
...
stats_connectome(derivatives_dir,ntcks,algorithm="tfnbs",nonstationarity=False,permute=False) # requires design_matrix and contrast files to be defined in "template" directory
