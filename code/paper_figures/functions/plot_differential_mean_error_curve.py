...
def mean_upper_triangle(mat):
    ...
    # If there are no valid entries, return NaN (caller will handle)
    return np.nan if vals.size == 0 else vals.mean()

def plot_differential_mean_error_curve(errors, x_label, y_label, title, ylim=None,legend=True, total_error=False):
    """
    ...
    errors : tuple of dict
        ...
        (errors_true_effect, errors_true_no_effect, errors_falzse),
        where each dictionary maps regularisation_strength -> error_matrix.
        Each error_matrix is a 2D array representing errors of each bundles for that category and regularisation strength.
    ...
    if total_error:
        ...
        total_sums = [
            mean_upper_triangle(errors_true_effect[s]) +
            mean_upper_triangle(errors_true_no_effect[s]) +
            mean_upper_triangle(errors_false[s])
            for s in strengths
        ]
    ...
    true_effect_sums = [mean_upper_triangle(errors_true_effect[strength]) for strength in strengths]
    true_no_effect_sums = [mean_upper_triangle(errors_true_no_effect[strength]) for strength in strengths]
    false_sums = [mean_upper_triangle(errors_false[strength]) for strength in strengths]

    def plot_category_spline(xs, ys, color, label,
                            linestyle='-', linewidth=10, alpha=0.7):
